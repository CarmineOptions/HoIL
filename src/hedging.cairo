...
use hoil::carmine::{
    available_strikes, buy_option, price_option, close_option_position, settle_option_position,
    CarmOptionWithSize
};
use hoil::helpers::{
    convert_from_Fixed_to_int, convert_from_int_to_Fixed, reverse, pow, toU256_balance,
    closest_value
};
use hoil::constants::{
...
fn iterate_strike_prices(
    ...
    loop {
        ...
        // If both strikes are above (in case of puts) or below (in case of calls) current price, no
        // point – so throw away.
...
fn iterate_strike_prices_with_bound(
    ...
    loop {
        ...
        // This is handled by the other type of the options.
        let tobuy = *strike_prices.at(i);
        // Handle the last available strike
        if (i + 1 == strike_prices.len()) {
            // Only append if bound is different from the last strike price
            if tobuy != bound {
                res.append((tobuy, bound));
            }
            break;
        }
        let tohedge = *strike_prices.at(i + 1);
...
// Calculates how much to buy at buystrike to get specified payoff at hedgestrike. Payoff is in
// quote token for puts, base token for calls.
...
fn adjust_for_rounding_condition(
    amount: u128, strike_price: Fixed, base_token_decimals: u8, quote_token_decimals: u8
) -> u128 {
...
fn price_options_at_strike_to_hedge_at(
    ...
    if (quote_token_addr.into() == TOKEN_USDC_ADDRESS && !calls) {
        let adj_notional = adjust_for_rounding_condition(
            notional, to_buy_strike, 18, 6
...
fn hedge_finalize(
    token_id: u256, close_option_position: bool, pail_token_address: ContractAddress
) -> ContractAddress {
    let caller = get_caller_address();
    let contract_address = starknet::get_contract_address();
    let hedge_token_dispatcher = IHedgeTokenDispatcher { contract_address: pail_token_address };

    // Transfer the hedge token from caller to this contract
    hedge_token_dispatcher
        .safe_transfer_from(caller, contract_address, token_id, ArrayTrait::new().span());
    ...
    // execute hedge token burn, that will result in transfering all assigned option tokens to
    // current contract
